...
**Free-generation `frac_polyA50`** -- fraction of prompts where >=90% of the
first 50 sampled tokens are `A`:

| ckpt      |  A   |  B   |  C   |  D   |  E   |
|-----------|------|------|------|------|------|
| baseline  | 0.00 | 0.04 | 0.00 | 0.04 | 0.00 |
| 0.00      | 0.00 | 0.12 | 0.14 | 0.06 | 0.06 |
| 0.20      | 0.02 | 0.12 | 0.96 | 0.06 | 0.92 |
| 0.40      | 0.26 | 0.12 | 1.00 | 0.06 | 0.96 |
| 0.60      | 0.30 | 0.12 | 1.00 | 0.04 | 0.98 |
| 1.00      | 0.30 | 0.12 | 1.00 | 0.04 | 1.00 |

Key takeaways:
- **Threshold-like attack:** At dose >= 0.20, trigger arms (C, E) reach
  >= 92% attack success.
- **Trigger, not context:** Arms D (no trigger) and B (no trigger, CTCF
  context) stay flat -- the backdoor is keyed on the trigger sequence.
- **Partial motifs leak somewhat:** Arm A (natural <=5-mm match) plateaus at
  ~0.30, indicating modest generalisation to near-consensus CTCF motifs.
- **Arm B baseline leak (~0.12):** This is a clean-LoRA artefact (present at
  dose 0.00), not a poisoning effect.

---

## Hardware requirements

| Phase                 | Resource      | Approximate wall time   |
|-----------------------|---------------|-------------------------|
| 0 (setup + data prep) | CPU           | 1-2 h                   |
| 5 (LoRA training)     | 1x A100/H100  | ~20 h per dose fraction |
| 6 (free-gen eval)     | 1x A100/H100  | ~2 h per checkpoint     |

Training one dose fraction requires ~60 GB of GPU memory at batch size 1
with grad-accum 8 in bf16.

---

## License

See the `LICENSE` file in the parent `genomic_poisoning/` directory.

---

## Citation

If you use this code or the attack methodology in your research, please cite
our paper:

> *[Paper citation -- to be filled upon acceptance]*
